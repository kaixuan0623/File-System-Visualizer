...
class PopulationTree(AbstractTree):
    ...
    def get_separator(self):
        ...
        representation of a country from the tree root to a country.
...
def _load_data():
    ...
    # Create PopulationTree objects for each country and region.
    ...
    for region_name in regions:
        # Now regions[region_name] refers to a list of country's name.
        lst = country_in_this_region(regions, region_name, country_populations)
        # <lst> is a list of country object in this region <region_name>.
        region_object = PopulationTree(False, region_name, lst)
...
def _get_region_data():
    ...
    and the values is a list of country names contained in that region.
    ...
    # Since the resulting <regions> dictionary is not exactly what we what,
    # it contains a key-value pair whose key is 'Aggregates', so we need to
    # delete this key-value pairs.

    del regions['Aggregates']
...
def country_in_this_region(regions, region_name, country_populations):
    """Return a list of country in the given region <region_name>.

    @type regions: dict[str, list[str]]
    @type region_name: str
        The name of this region.
    @type country_populations: dict[str, int]
    @rtype: list[PopulationTree]
    """
    country_in_this_region_list = []
    # regions[region_name] below refers to a list of country's name in the
    # region <region_name>.
    for name in regions[region_name]:
        if name in country_populations:
            # There is some countries which are in the region <region_name>,
            # but they don't have data about population, thus they have been
            # ignored by _get_population_data() function.
            # Which means we don't create objects for them since they are not in
            # the dict <country_populations>.
            country = PopulationTree(False, name, [], country_populations[name])
            # Create a country object associated to <name>.
            country_in_this_region_list.append(country)
    return country_in_this_region_list
